 ...
 */
//动态数据加载
function dynamidataLoading() {
    //订单数据
    $.getJSON("../json/orderData.json",function (response) {
    ...
    //我的出售
    $.getJSON("../json/sell.json",function (response) {
...
function personal() {
    ...
    //点击我的出售降价申请跳转到我要出售
    $(".sellInfo_baseCtrl ").on("click",".application",function () {
    ...
    $(".button_bt").on("click",function () {
        $(".pesnInfo-choose li").eq(1).children("a").trigger("click")
    });
    //点击我的订单返回按钮到个人信息
    $(".large_btn").on("click",function () {
        $(".pesnInfo-choose li").eq(0).children("a").trigger("click")
    });
    //点击降价申请我要出售按钮跳转到我的出售
